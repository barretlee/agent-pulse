...
Agent Pulse 把能力沉淀与页面呈现分开管理。当前维护 32 项 sensing / understanding / intelligence / experience / delivery / governance 能力，每项记录状态、成熟度、首次 release 与实现证据。后台 Evaluation Center 从 10 个维度生成 Scorecard，包含一手来源归属；样本不足会明确标注且不参与综合分。详见 [能力图谱](docs/CAPABILITIES.md)。
...
v0.3.0 的 Source Catalog 包含 **195 个**高价值候选源，覆盖 14 类、54 个中国来源与 141 个全球/海外来源。目录不等于全部已经接入：当前只启用 6 个，其余按 candidate / manual / restricted 进入验证队列，必须通过 contract fixture 与 shadow run 才能晋级。
